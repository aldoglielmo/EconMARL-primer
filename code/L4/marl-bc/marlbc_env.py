...
class RBCParallelEnv(ParallelEnv):
    ...
    def __init__(
        ...
        # unify agent count (KL uses self.n, KS now uses self.n)
        self.N = getattr(self.simulator, "n_agents", getattr(self.simulator, "n"))

        # list of agent names: h_0, h_1, …
        self.possible_agents = [f"h_{r}" for r in range(self.N)]
    ...
    def reset(self, seed=None, options=None):
        ...
        # === FIX 4: ensure attributes exist for default obs (KL at reset)
        if not hasattr(self.simulator, "wealths"):
            self.simulator.wealths = getattr(self.simulator, "ks", np.ones(self.N)).copy()
        if not hasattr(self.simulator, "incomes"):
            self.simulator.incomes = np.zeros(self.N)
        if not hasattr(self.simulator, "ls"):
            self.simulator.ls = np.ones(self.N)        
    ...
    def _get_observations(self):
        """
        Build a dict of observations for every agent.
        Vectorized: fetches each obs variable once, then slices per agent.
        """
        obs_matrix = np.column_stack([
            self._get_observations_from_simulator(space)
            for space in self.obs_space
        ])  # shape (N, obs_len)
    ...
    def _get_rewards(self):
        ...
        # compute Gini
        # gini = _compute_gini(self.simulator.ks)
    ...
    def step(self, actions):
        ...
        # check if simulator signaled early termination (e.g., high Gini)
        sim_terminated = False
        terminations = {agent: sim_terminated for agent in self.agents}
        ...
        # if episode terminated or truncated, clear agent list
        if env_truncation or sim_terminated:
            self.agents = []
